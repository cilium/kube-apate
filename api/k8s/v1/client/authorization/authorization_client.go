...
func (a *Client) GetAuthorizationAPIGroup(params *GetAuthorizationAPIGroupParams) (*GetAuthorizationAPIGroupOK, error) {
	...
	if params == nil {
		params = NewGetAuthorizationAPIGroupParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "getAuthorizationAPIGroup",
		Method:             "GET",
		PathPattern:        "/apis/authorization.k8s.io/",
		...
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &GetAuthorizationAPIGroupReader{formats: a.formats},
	...
	success, ok := result.(*GetAuthorizationAPIGroupOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for getAuthorizationAPIGroup: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
