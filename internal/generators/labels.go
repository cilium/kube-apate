...
func CiliumIdentityLabels(idx int64) map[string]string {
	podLabels := PodLabels(idx)
	podLabels["io.cilium.k8s.policy.cluster"] = "default"
	podLabels["io.cilium.k8s.policy.serviceaccount"] = "default"
	podLabels["io.kubernetes.pod.namespace"] = NamespaceName(idx)
	return podLabels
}
